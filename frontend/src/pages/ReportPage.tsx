...
//         )
//     );
//     handleModalClose(); // Close the modal after update
// };
...
//                       </td>
//                       <td className="border px-4 py-2 capitalize">{report.order_type}</td>
    ...
    <Layout>
      <div className="p-4">
        <div className="flex flex-col">
          <div className="flex flex-col space-y-4 mb-4">
            <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4 w-full md:w-auto">
              <button
                onClick={() => setReportType("sales")}
                className={`w-full md:w-auto p-4 rounded-lg shadow-md cursor-pointer ${reportType === "sales" ? "bg-purple-500 text-white" : "bg-gray-200"
                  }`}
              >
                Sales Report
              </button>
              <button
                onClick={() => setReportType("mess")}
                className={`w-full md:w-auto p-4 rounded-lg shadow-md cursor-pointer ${reportType === "mess" ? "bg-purple-500 text-white" : "bg-gray-200"
                  }`}
              >
                Mess Report
              </button>
              <button
                ...
                className={`w-full md:w-auto p-4 rounded-lg shadow-md cursor-pointer ${reportType === "product" ? "bg-purple-500 text-white" : "bg-gray-200"
                  }`}
              >
                Product Report
              </button>
              <button
                onClick={handleShowCancelledOrdersClick}
                className={`w-full md:w-auto p-4 rounded-lg shadow-md cursor-pointer ${showCancelledOrders ? "bg-red-500 text-white" : "bg-gray-200"
                  }`}
              >
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <button
            onClick={() => handleButtonClick("All")}
            className={`p-2 rounded w-auto ${activeButton === "All" ? "bg-blue-500 text-white" : "bg-gray-200"
              }`}
          >
            All
          </button>
            ...
            <>
              {["Dining", "Takeaway", "Delivery", "Cash", "Bank", "Cash-Bank", "Credit", "Delivered"].map((type) => (
                <button
                  key={type}
                  onClick={() => handleButtonClick(type)}
                  className={`p-2 rounded w-auto md:w-auto sm:w-1/2 ${activeButton === type ? "bg-blue-500 text-white" : "bg-gray-200"
                    }`}
                >
                  {type}
                </button>
            ...
            <>
              {[
                ...
                <button
                  key={type}
                  onClick={() => handleButtonClick(type)}
                  className={`p-2 rounded w-full md:w-auto sm:w-1/3 ${activeButton === type ? "bg-blue-500 text-white" : "bg-gray-200"
                    }`}
                >
                  {type}
                </button>
              ...
              <tr>
                ...
                {reportType === "product" && (
                  <>
                    ...
                    <th className="py-2 px-4 bg-gray-200">Total Quantity</th>
                    <th className="py-2 px-4 bg-gray-200">Total Amount</th>
                    <th className="py-2 px-4 bg-gray-200">Invoice Number</th>
                    <th className="py-2 px-4 bg-gray-200">Order Created At</th>
                    <th className="py-2 px-4 bg-gray-200">Order Type</th>
                    <th className="py-2 px-4 bg-gray-200">Cash Amount</th>
                    <th className="py-2 px-4 bg-gray-200">Bank Amount</th>
                    <th className="py-2 px-4 bg-gray-200">Payment Method</th>
            ...
            <tbody>
              {reportType === "sales" &&
                paginatedReports.map((report) => (
                  <tr key={report.id}>
                    ...
                    <td className="border px-4 py-2">{format(new Date(report.created_at), "dd-MM-yyyy")}</td>
                    <td className="border px-4 py-2 capitalize">{report.order_type}</td>
                    <td className="border px-4 py-2 capitalize">{report.payment_method}</td>
                    <td className="border px-4 py-2 capitalize">{report.status}</td>
                    ...
                    <td className="border px-4 py-2">
                      <button onClick={() => handleSalesMobileClick(report)} className="text-blue-500 hover:underline">
                        <Eye />
                      </button>
                    </td>
                    <td className="border px-4 py-2">
              ...
              {reportType === "mess" &&
                paginatedMessReports.map((report) => (
                  <tr key={report.id}>
                    ...
                    <td className="border px-4 py-2">{format(new Date(report.start_date), "dd-MM-yyyy")}</td>
                    <td className="border px-4 py-2">{format(new Date(report.end_date), "dd-MM-yyyy")}</td>
                    <td className="border px-4 py-2">{report.payment_method}</td>
                    <td className="border px-4 py-2">{report.status}</td>
                    <td className="border px-4 py-2">
                      <button onClick={() => handleMobileClick(report)} className="text-blue-500 hover:underline">
                        <Eye />
                      </button>
                    </td>
                    <td className="border px-4 py-2">
              ...
              {reportType === "product" &&
                paginatedProductReports.map((report, index) => (
                  <tr key={index}>
                    <td className="border px-4 py-2">{report.product_name}</td>
                    <td className="border px-4 py-2">{report.total_quantity}</td>
                    <td className="border px-4 py-2">{report.total_amount}</td>
                    <td className="border px-4 py-2">{report.invoice_number}</td>
                    <td className="border px-4 py-2">{format(new Date(report.order_created_at), "dd-MM-yyyy")}</td>
                    <td className="border px-4 py-2 capitalize">{report.order_type}</td>
                    <td className="border px-4 py-2">{report.cash_amount}</td>
                    <td className="border px-4 py-2">{report.bank_amount}</td>
                    <td className="border px-4 py-2 capitalize">{report.payment_method}</td>
        ...
        <div className="mt-4 p-4 bg-gray-100 rounded-lg shadow-md">
          ...
          <div className="flex justify-end space-x-4 mt-2">
            <p className="font-bold text-lg">Bank Amount:</p>
            <p className="font-bold text-lg">₹{totalBankAmount.toFixed(2)}</p>
          </div>
          <div className="flex justify-end space-x-4 mt-2">
            <p className="font-bold text-lg">Credit Amount:</p>
            <p className="font-bold text-lg">₹{totalCreditAmount.toFixed(2)}</p>
          </div>
          <div className="flex justify-end space-x-4 mt-2">
            <p className="font-bold text-lg">Total Amount:</p>
            <p className="font-bold text-lg">₹{totalAmount.toFixed(2)}</p>
